...
class Admin {
	...
	public function admin_notices(): void {
		...
		if ( Firewall::is_cf_notice_ready() ) {
			...
			$header = __( 'Cloudflare Usage Detected: Switched to CF-Connecting-IP for Better Compatibility', 'wpdef' );
		} elseif ( Firewall::is_xff_notice_ready() ) {
			$is_show = 'xff';
			$class_notice = 'notice-warning';
			$header = __( 'Improve IP Detection : We suggest Switching to X-Forward-For IP Detection Method', 'wpdef' );
		}
