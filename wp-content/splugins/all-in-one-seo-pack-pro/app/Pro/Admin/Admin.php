...
class Admin extends CommonAdmin\Admin {
	...
	public function scheduleUnescapeData() {
		...
		aioseo()->cache->update( 'unslash_escaped_data_terms', time(), WEEK_IN_SECONDS );
	...
	public function unslashEscapedDataTerms() {
		$termsToUnslash = 200;
		$timeStarted    = gmdate( 'Y-m-d H:i:s', aioseo()->cache->get( 'unslash_escaped_data_terms' ) );
		...
		if ( empty( $terms ) ) {
			aioseo()->cache->delete( 'unslash_escaped_data_terms' );
