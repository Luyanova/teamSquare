...
class TermMeta {
	...
	public function scheduleImport() {
		try {
			...
			if ( ! aioseo()->cache->get( 'import_term_meta_yoast_seo' ) ) {
				aioseo()->cache->update( 'import_term_meta_yoast_seo', time(), WEEK_IN_SECONDS );
			}
	...
	public function importTermMeta() {
		if ( ! aioseo()->db->tableExists( 'yoast_indexable' ) ) {
			aioseo()->cache->delete( 'import_term_meta_yoast_seo' );
		...
		$timeStarted      = gmdate( 'Y-m-d H:i:s', aioseo()->cache->get( 'import_term_meta_yoast_seo' ) );
