...
class TermMeta {
	...
	public function scheduleImport() {
		try {
			...
			if ( ! aioseo()->cache->get( 'import_term_meta_rank_math' ) ) {
				aioseo()->cache->update( 'import_term_meta_rank_math', time(), WEEK_IN_SECONDS );
			}
	...
	public function importTermMeta() {
		...
		$timeStarted      = gmdate( 'Y-m-d H:i:s', aioseo()->cache->get( 'import_term_meta_rank_math' ) );
		...
		if ( ! $terms || ! count( $terms ) ) {
			aioseo()->cache->delete( 'import_term_meta_rank_math' );
