...
class Updates extends CommonMain\Updates {
	...
	public function runUpdates() {
		...
		if ( version_compare( $lastActiveVersion, '4.0.0', '>=' ) && version_compare( $lastActiveVersion, '4.0.5', '<' ) ) {
			try {
				aioseo()->cache->update( 'v4_migrate_post_og_image', time(), WEEK_IN_SECONDS );
				aioseo()->cache->update( 'v4_migrate_term_og_image', time(), WEEK_IN_SECONDS );
	...
	public function runPreAddonUpdates() {
		parent::runPreAddonUpdates();

		$lastActiveVersion = aioseo()->internalOptions->internal->lastActiveVersion;
		if ( version_compare( $lastActiveVersion, '4.1.8', '<' ) ) {
			// Re-activate the license key before addons load.
	...
	public function migratePostOgImage() {
		...
		$timeStarted     = gmdate( 'Y-m-d H:i:s', aioseo()->cache->get( 'v4_migrate_post_og_image' ) );
		...
		if ( ! $postsToMigrate || ! count( $postsToMigrate ) ) {
			aioseo()->cache->delete( 'v4_migrate_post_og_image' );
	...
	public function migrateTermOgImage() {
		...
		$timeStarted      = gmdate( 'Y-m-d H:i:s', aioseo()->cache->get( 'v4_migrate_term_og_image' ) );
		...
		if ( ! $termsToMigrate || ! count( $termsToMigrate ) ) {
			aioseo()->cache->delete( 'v4_migrate_term_og_image' );
	...
	private function removeUnusedColumns() {
		...
		foreach ( $columnsToRemove as $columnName ) {
			if ( aioseo()->db->columnExists( 'aioseo_terms', $columnName ) ) {
				$tableName = aioseo()->db->db->prefix . 'aioseo_terms';
				aioseo()->db->execute(
					"ALTER TABLE {$tableName}
	...
	protected function migrateOgTwitterImageColumns() {
		...
		if ( aioseo()->db->tableExists( 'aioseo_terms' ) ) {
			...
			if ( ! aioseo()->db->columnExists( 'aioseo_terms', 'twitter_image_url' ) ) {
				aioseo()->db->execute(
