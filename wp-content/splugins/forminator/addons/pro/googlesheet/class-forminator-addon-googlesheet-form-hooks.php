	...
	public function get_status_on_create_row( $connection_id, $submitted_data, $connection_settings, $form_entry_fields ) {
		...
		try {
			...
			foreach ( $header_fields as $element_id => $header_field ) {
				...
				$is_date_field = apply_filters( 'wpmudev_fm_is_date_field', 'date' === $field_type, $form_value, $element_id, $form_id );
				if( $field_type === 'number' || $is_date_field || is_numeric( $form_value ) ){
					if( $is_date_field ){
						$form_value = apply_filters( 'wpmudev_fm_date_value', strtotime( $form_value ), $form_value, $element_id, $form_id );
						$form_value = intval( ( $form_value - strtotime('1899/12/30') )/DAY_IN_SECONDS );
					}
